...
namespace TrainingDay.Old
{
    ...
    {
        public ExerciseListPageViewModel vmParent { get; set; }
        // need finish filter to parent page
        public FilterPage(ExerciseListPageViewModel parent)
        {
            InitializeComponent();
            vmParent = parent;
            //new
            ...
            BindingContext = this;
            MusclesListView.ItemsSource = itemsMuscles;
        }
        ...
        {
            scaleX = (float)(MuscleImage.Width / 496);
            scaleY = (float)(MuscleImage.Height / 514);
            ...
            foreach (var currentMuscle in currentMuscles)
            {
                DrawMuscles(currentMuscle);
            }

            skiaView.HeightRequest = MuscleImage.Height;
            //MainGrid.RowDefinitions[0].Height = new GridLength(MuscleImage.Height);
        }
        ...
        List<MusclesEnum> currentMuscles = new List<MusclesEnum>();
        private void NewItem_PropertyChanged(object sender, System.ComponentModel.PropertyChangedEventArgs e)
        {
            ...
            {
                if (item.IsChecked)
                    currentMuscles.Add(item.Muscle);
                else
                    currentMuscles.Remove(item.Muscle);
            }

            skiaView.InvalidateSurface();
        }

        private void SelectMuscles_Click(object sender, EventArgs e)
        {
